...
func main() {
	...
	// connect
	if _, e := c.Write(protocol.NewPacket(uint8(pb.ID_MSG_Connect), &pb.C2S_ConnectMsg{
	...
	// ready
	if _, e := c.Write(protocol.NewPacket(uint8(pb.ID_MSG_JoinRoom), nil).Serialize()); nil != e {
		panic(fmt.Sprintf("write error:%s", e.Error()))
	}
	time.Sleep(time.Second)
	// ready
	if _, e := c.Write(protocol.NewPacket(uint8(pb.ID_MSG_Ready), nil).Serialize()); nil != e {
		panic(fmt.Sprintf("write error:%s", e.Error()))
	}
	time.Sleep(time.Second)
	// write
	for i := 0; i < 10; i++ {
		p := protocol.NewPacket(uint8(pb.ID_MSG_Input), &pb.C2S_InputMsg{
